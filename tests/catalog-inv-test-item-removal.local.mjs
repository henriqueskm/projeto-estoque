...
if (!database || database === "postgres" || !database.startsWith("nk_prepare7inv_")) {
  throw new Error(
    "CATALOG_INV_TEST_DB_NAME must name an nk_prepare7inv_ disposable database; the baseline database is never mutated.",
  );
}
...
console.log("ALVO CONFIRMADO: POSTGRESQL LOCAL/DESCARTAVEL PARA PREPARACAO 7 INV");
...
assert.equal(
  scalar("select exists(select 1 from supabase_migrations.schema_migrations where version = '20260913003000')"),
  "f",
);
...
const legitimateCatalogBefore = catalogSnapshot();

psql(`update public.items set code = '7 INV TEST' where id = '${targetItemId}'::uuid`);
expectPrepareFailure(/exact identity or audit fields changed/i);
psql(`update public.items set code = '7 INV' where id = '${targetItemId}'::uuid`);

psql(`update public.items set description = 'DESCRICAO DIVERGENTE' where id = '${targetItemId}'::uuid`);
expectPrepareFailure(/exact identity or audit fields changed/i);
psql(`update public.items set description = 'SERVO BR-040 INVER SEM KIT' where id = '${targetItemId}'::uuid`);

psql(`update public.items set created_by = '${divergentCreatorId}'::uuid where id = '${targetItemId}'::uuid`);
expectPrepareFailure(/exact identity or audit fields changed/i);
psql(`update public.items set created_by = '${expectedCreatorId}'::uuid where id = '${targetItemId}'::uuid`);

psql(`update public.items set created_by_name_snapshot = 'Henrique K.' where id = '${targetItemId}'::uuid`);
expectPrepareFailure(/exact identity or audit fields changed/i);
psql(`update public.items set created_by_name_snapshot = 'Henrique Klein' where id = '${targetItemId}'::uuid`);

psql(`insert into public.stock_balances (item_id, quantity) values ('${targetItemId}'::uuid, 0)`);
expectPrepareFailure(/public\.stock_balances\.item_id contains 1 reference/i);
assert.equal(number(`select count(*) from public.stock_balances where item_id = '${targetItemId}'::uuid`), 1);
psql(`delete from public.stock_balances where item_id = '${targetItemId}'::uuid`);

psql(`delete from public.loose_parts where item_id = '${targetItemId}'::uuid`);
expectPrepareFailure(/loose-part subtype is missing or duplicated/i);
psql(`insert into public.loose_parts (item_id) values ('${targetItemId}'::uuid)`);

const duplicateSubtypeFailure = psql(
...
assert.equal(number(`select count(*) from public.loose_parts where item_id = '${targetItemId}'::uuid`), 1);

const itemIdsBefore = itemIds();
applySql(prepareMigrationSql);
const itemIdsAfter = itemIds();
assert.deepEqual(
  itemIdsBefore.filter((itemId) => !itemIdsAfter.includes(itemId)),
  [targetItemId],
);
assert.deepEqual(
  itemIdsAfter.filter((itemId) => !itemIdsBefore.includes(itemId)),
  [],
);
...
applySql(policyMigrationSql);
assert.equal(catalogSnapshot(), legitimateCatalogBefore);
...
console.log("EXACT_IDENTITY_AND_REFERENCE_GUARDS_VERIFIED");
console.log("PREPARE_THEN_HISTORICAL_POLICY_SEQUENCE_VERIFIED");
