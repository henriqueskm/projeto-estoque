...
export const configurationDisassemblyProposalLifetimeSeconds = 10 * 60;
const tokenVersion = 1 as const;
const maximumTokenLength = 4096;
const maximumInteger = 2_147_483_647;
const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export type ConfigurationDisassemblyProposalPayload = {
...
type VerificationResult =
  | { ok: true; payload: ConfigurationDisassemblyProposalPayload }
  | { ok: false; reason: "expired"; payload: ConfigurationDisassemblyProposalPayload }
...
function parsePayload(value: unknown): ConfigurationDisassemblyProposalPayload | null {
  ...
  if (Object.keys(value).length !== keys.length || Object.keys(value).some((key) => !keys.includes(key)) ||
    value.version !== tokenVersion || value.action !== "configuration_disassembly" ||
    ...
    value.expiresAt <= value.issuedAt || value.expiresAt - value.issuedAt > configurationDisassemblyProposalLifetimeSeconds) return null;
  return { version: tokenVersion, action: "configuration_disassembly", userId: value.userId.toLowerCase(),
...
export function createConfigurationDisassemblyProposalToken(
  input: Omit<ConfigurationDisassemblyProposalPayload, "version" | "action" | "issuedAt" | "expiresAt">,
  secret: string,
  now = new Date(),
) {
  if (secret.trim().length < 32) return null;
  const issuedAt = Math.floor(now.getTime() / 1000);
  const payload = parsePayload({ version: tokenVersion, action: "configuration_disassembly", ...input,
    issuedAt, expiresAt: issuedAt + configurationDisassemblyProposalLifetimeSeconds });
  if (!payload) return null;
  const encoded = Buffer.from(JSON.stringify(payload), "utf8").toString("base64url");
  return { payload, token: `${encoded}.${signature(encoded, secret).toString("base64url")}` };
}

export function verifyConfigurationDisassemblyProposalToken(token: string, secret: string, userId: string, now = new Date()): VerificationResult {
