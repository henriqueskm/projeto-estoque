...
begin
  ...
  -- Ordered upserts make concurrent first-balance creation deterministic.
  -- ON CONFLICT waits for a concurrent creator before the locks below read
  -- the final current balance.
...
grant execute on function public.stock_inbound_items(jsonb, text)
to authenticated;
