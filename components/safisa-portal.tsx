...
function Metric({ label, value, tone = "neutral" }: { label: string; value: number; tone?: "neutral" | "blue" | "green" }) {
  ...
  return (
    <div className={`min-w-0 rounded-xl border px-3 py-2 ${toneClass}`}>
      <dt className="text-[0.68rem] font-black tracking-wide text-slate-600 uppercase">{label}</dt>
      <dd className="mt-0.5 text-xl font-black tabular-nums text-slate-950">{numberFormatter.format(value)}</dd>
    </div>
  );
}
                ...
                {selectedOrder.lines.map((line) => (
                  <article key={line.supplierOrderItemId} className="rounded-2xl border border-slate-200 p-4 sm:p-5">
                    ...
                    <dl className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                      ...
                      <Metric label="Já pronto" value={line.readyQuantity} tone="blue" />
                      <Metric label="Retirado" value={line.pickedQuantity} tone="green" />
                      <Metric label="Restante" value={line.waitingReadyQuantity} />
                    </dl>
                    ...
                    {!selectedOrder.isReadOnly && line.waitingReadyQuantity > 0 ? (
                      <div className="mt-4 border-t border-slate-200 pt-4">
                        ...
                        >
                          <div className="flex-1"><label htmlFor={`quantity-${line.supplierOrderItemId}`} className="mb-1.5 block text-sm font-black text-slate-800">Novas unidades prontas</label><input id={`quantity-${line.supplierOrderItemId}`} name="quantity" type="number" inputMode="numeric" min="1" max={line.waitingReadyQuantity} step="1" required disabled={isPending} className="min-h-11 w-full rounded-xl border border-slate-300 px-3 text-base font-bold outline-none focus:border-blue-700 focus:ring-3 focus:ring-blue-100 disabled:bg-slate-100" /></div>
                          <button type="submit" disabled={isPending} className="min-h-11 rounded-xl bg-blue-800 px-4 text-sm font-black text-white transition hover:bg-blue-900 focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-blue-700 disabled:cursor-wait disabled:opacity-60">{activeLineId === line.supplierOrderItemId && isPending ? "Salvando..." : "Informar como pronto"}</button>
                        </form>
                        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <button type="button" disabled={isPending} onClick={() => setConfirmation({ kind: "remaining", line })} className="min-h-11 rounded-xl border border-blue-300 bg-white px-4 text-sm font-black text-blue-900 transition hover:bg-blue-50 focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-blue-700 disabled:opacity-50">Marcar restante como pronto</button>
