...
const maximumDistinctLines = 500;
const uuidPattern =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

type NormalizedOutboundRequest = {
...
type RpcReceipt = {
  ...
  auto_assembled_quantity?: unknown;
};

function invalidRequest(error: string): OutboundActionResult {
  return { ok: false, error };
}

function normalizeRequest(
  input: unknown,
): NormalizedOutboundRequest | OutboundActionResult {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    return invalidRequest("Os dados da saída são inválidos.");
  }
  ...
  if (!Array.isArray(request.p_lines) || request.p_lines.length === 0) {
    return invalidRequest("Adicione pelo menos uma linha à saída.");
  }

  if (request.p_lines.length > maximumDistinctLines) {
    return invalidRequest("A saída possui linhas demais para uma operação.");
  }

  const linesByKey = new Map<string, OutboundRequestLine>();
  ...
  for (const rawLine of request.p_lines) {
    ...
    if (
      ...
      return invalidRequest("Revise os itens e códigos comerciais informados.");
    }
...
function parseReceipt(data: unknown): OutboundReceipt | null {
  ...
  if (
    ...
    typeof receipt.auto_assembled_quantity !== "number" ||
    !Number.isSafeInteger(receipt.auto_assembled_quantity) ||
  ...
  return {
    ...
    autoAssembledQuantity: receipt.auto_assembled_quantity,
  };
}
...
function mapRpcError(code: string | undefined, message: string) {
  ...
  if (
    ...
    return "Esta tentativa já foi usada com uma saída diferente. Volte, revise os dados e tente novamente.";
  }
  ...
  if (
    ...
    return "O estoque mudou ou ficou insuficiente desde a revisão. Volte e confira os saldos antes de tentar novamente.";
  }
  ...
  if (
    ...
    return "Um item ou código comercial não está mais disponível. Atualize a página e revise a saída.";
  }
  ...
  try {
    ...
    if (itemIds.length > 0) {
      ...
      if (
        ...
        return invalidRequest(
          "Um ou mais itens físicos não estão disponíveis para saída.",
        );
      }
    }
    ...
    if (commercialCodeIds.length > 0) {
      ...
      if (!data || error || data.length !== commercialCodeIds.length) {
        return invalidRequest(
          "Um ou mais códigos comerciais não estão disponíveis para saída.",
        );
      }
    ...
    if (componentIds.length > 0) {
      ...
      if (!componentsAreValid) {
        return invalidRequest(
          "Os componentes de uma configuração não estão disponíveis para saída.",
        );
      }
    }
    ...
    if (!receipt) {
      return invalidRequest(
        "A saída foi processada, mas o comprovante não pôde ser carregado. Atualize a página antes de tentar novamente.",
      );
    }
