...
export function StatisticsConversationScene() {
  ...
  const scroll = interpolate(frame, [0, 210, 232, 360], [0, 0, -220, -220], {
    ...
    <div style={{position: "absolute", inset: 0, opacity: fadeWindow(frame, sceneDuration, 24, 24), background: "radial-gradient(circle at 72% 15%, #2f3a41 0%, #171d21 58%, #0d1114 100%)"}}>
      <AssistantChrome>
        <div style={{position: "absolute", inset: 0, padding: "38px 54px 70px", translate: `0 ${scroll}px`}}>
          ...
          <ChatMessage side="assistant" style={{marginTop: 24, ...entrance(frame, fps, 60, 16)}}>
          ...
          <ChatMessage side="user" style={{marginTop: 24, ...entrance(frame, fps, 156, 16)}}>Sim</ChatMessage>
          <ChatMessage side="assistant" style={{marginTop: 24, ...entrance(frame, fps, 210, 16)}}>
            <div style={{width: 880, maxWidth: "100%", overflow: "hidden", border: `1px solid ${demoColors.border}`, borderRadius: 22, background: "#faf9f6"}}>
              <div style={{padding: "16px 20px", color: "#9a6a28", fontSize: 15, fontWeight: 950, letterSpacing: 2.1, ...entrance(frame, fps, 216, 12)}}>ESTATÍSTICAS · ÚLTIMOS {statistics.periodDays} DIAS</div>
              <div style={{padding: "0 20px 17px", fontSize: 27, fontWeight: 950, ...entrance(frame, fps, 222, 12)}}>Ranking · Servos com kit</div>
              <div style={{display: "grid", gridTemplateColumns: "58px 1fr 120px", alignItems: "center", gap: 16, padding: "18px 20px", borderTop: `1px solid ${demoColors.border}`, background: "white", ...entrance(frame, fps, 228, 12)}}>
