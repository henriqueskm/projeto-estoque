...
| MIG-SAF-001 | Fundação de banco do Portal da Safisa | Migration | NK-SAF-001; MIG-BASE-001 | C | BLOCKED_OPERATIONAL_TEST | `agent/safisa-portal-migration` | [#5](https://github.com/henriqueskm/projeto-estoque/pull/5) | Crítico | Gate de Pedidos ativos | Mesclada em `main`, mas não aplicada remotamente: 5 Pedidos legados ativos exigem MIG-SAF-003 |
| MIG-SAF-003 | Transição segura de Pedidos legados | Migration incremental | MIG-SAF-001 | C | WAITING_HUMAN_REVIEW | `agent/safisa-legacy-order-transition` | [#8](https://github.com/henriqueskm/projeto-estoque/pull/8) | Crítico | Revisão humana do SQL e dos testes locais | Revisar PR draft; aplicação remota permanece proibida |
