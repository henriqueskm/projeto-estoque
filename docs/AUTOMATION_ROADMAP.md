...
| NK-ORD-006 | Entrada no Estoque a partir do Pedido pela Assistente | Aplicação | Fluxo oficial de entrada por Pedido | B | WAITING_HUMAN_VISUAL_REVIEW | `agent/assistant-order-stock-entry` | Pendente | Crítico | Visual e operacional | Corrige a leitura de `waiting_stock_quantity`, mantém prévia/confirmação HMAC e reutiliza a Server Action oficial |
